...
// 语言类型
const LANG_ZAWGYI = 'my-MM';
const LANG_MM3 = 'my';
const LANG_ENGLISH = 'en';
const LANG_CHINESE = 'zh-Hans';
...
const transferToMm3IfNeeded=(str)=> {
  if (isEmpty(str)) {
    return '';
  }
...
const transferToZawgyiIfNeeded=(str)=> {
  if (isEmpty(str)) {
    return '';
  }
  if (getLanguage() === LANG_ZAWGYI) {
...
const getString = () => {
  ...
  let obj = strings.find(item=> item.name === language);
  if(!obj){
    return {};
  }else{
    return obj.strings;
...
import basic_en from '../strings/basic_en'
import basic_chinese from '../strings/basic_zh_Hans'
import basic_zawgyi from '../strings/basic_zawgyi'
import basic_mm3 from '../strings/basic_mm3'
let strings = [];
 ...
 * @param obj
 */
const registerString=(obj = {name,strings})=>{

  if(!obj || ! obj instanceof Object)return {name:LANG_ENGLISH,strings:basic_en};
  switch (obj.name) {
    case LANG_ZAWGYI:
      Object.assign(obj.strings,basic_zawgyi,obj.strings);
      break;
    case LANG_MM3:
      Object.assign(obj.strings,basic_mm3,obj.strings);
      break;
    case LANG_CHINESE:
      Object.assign(obj.strings,basic_chinese,obj.strings);
      break;
    case LANG_ENGLISH:
      Object.assign(obj.strings,basic_en,obj.strings);
      break;
    default:
      Object.assign(obj.strings,basic_en,obj.strings);
  }
...
const LanguageUtil = {
  ...
  getString,
  getStringByKey,
  transferToZawgyiIfNeeded,
  transferToMm3IfNeeded,
  mm3FromZawgyi,
  zawgyiFromMm3,
  getLanguage,
};
